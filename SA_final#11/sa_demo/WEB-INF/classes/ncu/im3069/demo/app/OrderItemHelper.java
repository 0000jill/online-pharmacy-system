...
public class OrderItemHelper {
    ...
    public JSONArray createByList(long order_id, List<OrderItem> orderproduct) {
        ...
        for(int i=0 ; i < orderproduct.size() ; i++) {
            ...
            try {
                ...
                /** SQL指令 */
                String sql = "INSERT INTO `sa_test`.`tbl_orderitem`(`order_id`, `product_id`, `price`, `quantity`, `subtotal`)"
                        + " VALUES(?, ?, ?, ?, ?)";
    ...
    public ArrayList<OrderItem> getOrderProductByOrderId(int order_id) {
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "SELECT * FROM `sa_test`.`tbl_orderitem` WHERE `tbl_orderitem`.`order_id` = ?";
            ...
            while(rs.next()) {
                ...
                /** 將 ResultSet 之資料取出 */
                int order_product_id = rs.getInt("order_product_id");
